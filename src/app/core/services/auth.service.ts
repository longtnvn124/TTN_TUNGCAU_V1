...
import {
  ...
  REFRESH_TOKEN,
  getRoute,
  SWITCH_DONVI_ID, getWsUrl, wsPath
} from '@env';
...
export type AppSocketEventName = 'close' | 'connect' | 'error' | 'batdauthi' | 'ketthucthi' | 'start_time' | 'start_shift';

export interface AppSocketEvent {
  name: AppSocketEventName;
  data: any
}
...
class AppSocket {
  ...
  private _observer: Subject<AppSocketEvent> = new Subject<AppSocketEvent>();
  observerStatus: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(false);
  ...
  connect(accessToken: string): void {
    ...
    if (accessToken) {
      ...
      this.socket.on('connect', () => {
        ...
        this._observer.next({name: 'connect', data: null});
        this.observerStatus.next(true);
        console.log('socket: connect');

        engine.on('close', () => {
          this._observer.next({name: 'close', data: null});
          this.observerStatus.next(false);
          console.log('socket: close');

        });
        engine.on('error', () => {
          this._observer.next({name: 'error', data: null});
          console.log('socket: error');

        });
        // engine.on('ketthucthi', (data) => {
        //   this._observer.next({name: 'ketthucthi', data})
        //   console.log('socket: ketthucthi')
        // });
        // engine.on('start_time', (data) => {
        //   console.log(data);
        //   this._observer.next({name: 'start_time', data})
        //   console.log('socket: start_time')
        // });
        // engine.on('batdauthi', (data) => {
        //   console.log( data);
        //   this._observer.next({name: 'batdauthi', data})
        //   console.log('socket: batdauthi')
        // });
        // engine.on('start_shift', (data) => {
        //   this._observer.next({name: 'start_shift', data})
        //   console.log('socket: start_shift')
        // });
      ...
      //
      // });
      this.socket.on('ketthucthi', (data) => {
        this._observer.next({name: 'ketthucthi', data})
        console.log('socket: ketthucthi')
      });
      ...
      this.socket.on('start_shift', (data) => {
        this._observer.next({name: 'start_shift', data})
        console.log('socket: start_shift')
      });
    }
  ...
  get observer(): Observable<AppSocketEvent> {
...
export class AuthService {
  ...
  readonly observerAppSocketStatus: BehaviorSubject<boolean>;
  ...
  constructor(
    ...
    this.appSocket = new AppSocket(this.accessToken);
    this.observerAppSocketStatus = this.appSocket.observerStatus;
  }

  onSocketResponse(): Observable<AppSocketEvent> {
    return this.appSocket.observer;
  }
  onsocKet():AppSocket {
  ...
  get refreshTokenActor() {
    return this.http.post<{
      data: string
    }>(getRoute('refresh-token'), {'refresh_token': this.refreshToken}).pipe(tap(({data}) => this.accessToken = data));
  }
  ...
  private startSession(info: Auth | Token): Observable<boolean> {
    ...
    this.appSocket.connect(this.accessToken );
